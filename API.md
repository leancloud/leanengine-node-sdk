...
可以在应用收到 `SIGTERM` 信号时调用 `AV.Cloud.close()`，使应用优雅退出：

```
process.on('SIGTERM', () => {
  console.log('Received SIGTERM. App will exit.');
  AV.Cloud.close();
});
```

更多关于信号量的说明见 [Node.js 文档](https://nodejs.org/api/process.html#process_signal_events)。
