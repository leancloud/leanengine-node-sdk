...
var classHook = function(className, hook, object, user, req, cb) {
  ...
  const isBeforeHook = hook.indexOf('__before_') === 0
  ...
  // for beforeUpdate and afterUpdate
  ...
  if (isBeforeHook && hookFunction.length === 2) {
    try {
      if (!isBeforeHook) {
    ...
    promiseTry(hookFunction.bind(null, request)).then( () => {
      if (!isBeforeHook) {
        cb(null, 'ok');
      } else if (hook === '__before_delete_for_') {
        cb(null, {}, true);
      } else {
        cb(null, obj, true);
      }
...
var onVerified = function(req, type, user) {
  ...
  promiseTry(Cloud.functions['__on_verified_' + type].bind(null, request)).catch( err => {
    if (!err.status) {
      err.status = 500;
    }
...
var onLogin = function(req, user, cb) {
  ...
  const onLoginHook = Cloud.functions['__on_login__User'];

  if (onLoginHook.length === 2) {
    const response = utils.prepareResponseObject(req.res, function(err) {
      if (err) {
        cb(new Error(err));
      } else {
        cb(null, 'ok');
    ...
    try {
      onLoginHook(request, response);
    } catch (err) {
      console.warn('Execute onLogin failed with error: ' + (err.stack || err));
      cb(err);
    }
  } else {
    promiseTry(onLoginHook.bind(this, request)).then( () => {
      cb(null, 'ok');
    }).catch( err => {
      if (!err.status) {
        err.status = 500;
...
var onCompleteBigQueryJob = function(result) {
  promiseTry(Cloud.functions['__on_complete_bigquery_job'].bind(null, result)).catch( err => {
    if (!err.status) {
      err.status = 500;
    }
