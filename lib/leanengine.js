...
function createCloudFunctionRouter(options) {
  ...
  cloudFunctions.use(function(req, res, next) {
    promiseTry( () => {
      ...
      } else if (functionOrClass === '_User' && _.contains(['userBeforeAuthDataMatch', 'userBeforeAuthDataCommit'], hookName)) {
        req.rpcCall = true;
        return callCloudFunction(req, utils.hookNameMapping[hookName], options);
