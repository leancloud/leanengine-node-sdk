...
AV._.extend(AV.Cloud, require('./cloud'));
...
Cloud.use(function(req, res, next) {
  // 兼容 connect
  if (!res.req) res.req = req;
  if (!req.res) req.res = res;

  var request = prepareRequestObject({req: req});
  var response = prepareResponseObject(res);

  req.currentUser = request.currentUser;
  req.getSessionToken = request.getSessionToken;

  res.currentUser = response.currentUser;
  res.saveCurrentUser = response.saveCurrentUser;
...
['1', '1.1'].forEach(function(apiVersion) {
  ['functions', 'call'].forEach(function(urlEndpoint) {
    ...
    Cloud.use(route, bodyParser.urlencoded({extended: false, limit: '20mb'}));
    Cloud.use(route, bodyParser.json({limit: '20mb'}));
    Cloud.use(route, bodyParser.text({limit: '20mb'}));
    ...
    Cloud.use(route, function(req, res) {
      ...
      if (split.length == 2) { // cloud function
        call(split[1], req.body, req.AV.user, req, {
        ...
        if (split[1] === 'onVerified') {
          ...
          onVerified(req, split[2], userObj);
          cb(null, 'ok');

        } else if (split[1] === '_User' && split[2] === 'onLogin') {
          userObj._finishFetch(req.body.object, true);
          onLogin(req, userObj, cb);
          ...
          classHook(split[1], hookNameMapping[split[2]], req.body.object, req.AV.user, req, cb);
        }
      }
    });
...
var prepareRequestObject = function(options) {
  ...
  return {
    ...
    currentUser: function() {
      return user || (req.AV && req.AV.user);
    },
...
var prepareResponseObject = function(res, callback) {
  return {
    ...
    /* The User whice will be saved in cookies */
    currentUser: function() {
      return res.user || (res.req.AV && res.req.AV.user);
    },

    saveCurrentUser: function(user) {
      if (res.req.sessionCookies) {
        res.user = user;
      } else {
        throw new Error('需要启用 Cookie Session 插件才能记录用户会话信息');
      }
    }
  };
};
