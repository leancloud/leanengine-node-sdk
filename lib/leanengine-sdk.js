...
// 健康监测 router
Cloud.use('/1/ping', function(req, res) {
  res.end('pong');
});

apiVersions = ['1', '1.1'];
apiVersions.forEach(function(apiVersion) {
...
var respError = function(res, err) {
  ...
  var error = 'null message';
  if (err) {
    if (toString.call(err) === '[object String]')
      error = err;
    else 
      error = err.message || err.responseText || err;
  }
...
var className = function(clazz) {
  if (toString.call(clazz) == '[object String]') {
    return clazz;
  }
  if (clazz.className)
...
var unauthResp = function(res) {
  res.statusCode = 401;
  res.setHeader('content-type', 'application/json; charset=UTF-8');
  return res.end(JSON.stringify({ code: 1, error: "Unauthorized." }));
};
